...
namespace ft
{
	...
	class vector_iterator : public ft::iterator<ft::random_access_iterator_tag, T>
	{
		...
		public:
			...
			vector_iterator(const vector_iterator& tmp) : ptr(tmp.get_ptr()) {};
			vector_iterator &operator=(const vector_iterator &tmp);
			~vector_iterator() {};
			vector_iterator &operator++();
			vector_iterator operator++(int);
			bool operator==(const vector_iterator<T> &tmp) const;
			bool operator!=(const vector_iterator<T> &tmp) const;
			bool operator==(const const_vector_iterator<T> &tmp) const;
			bool operator!=(const const_vector_iterator<T> &tmp) const;
			reference operator*();
			pointer operator->();
			vector_iterator &operator--();
	...
	template<class T>
	bool vector_iterator<T>::operator==(const vector_iterator<T> &tmp) const
	{
		return (ptr == tmp.get_ptr());
	}
	template<class T>
	bool vector_iterator<T>::operator==(const const_vector_iterator<T> &tmp) const
	{
		return (ptr == tmp.get_ptr());
	}
	template<class T>
	...
	class const_vector_iterator : public ft::iterator<ft::random_access_iterator_tag, T>
	{
		...
		public:
			...
			const_vector_iterator(const const_vector_iterator& tmp) : ptr(tmp.get_ptr()) {};
			const_vector_iterator(const vector_iterator<T>& other) : ptr(other.get_ptr()) {};
	...
	template<class T>
	bool const_vector_iterator<T>::operator==(const const_vector_iterator<T> &tmp) const
	{
		return (ptr == tmp.get_ptr());
	}
	template<class T>
	bool const_vector_iterator<T>::operator==(const vector_iterator<T> &tmp) const
	{
		return (ptr == tmp.get_ptr());
	}
	template<class T>
	...
	bool	operator ==	(const vector_iterator<iter>& lhs, const vector_iterator<iter>& rhs) { return (lhs.get_ptr() == rhs.get_ptr()); }
	template<typename iter>
	bool	operator !=	(const vector_iterator<iter>& lhs, const vector_iterator<iter>& rhs) { return (lhs.get_ptr() != rhs.get_ptr()); }
	template<typename iter>
	bool	operator <	(const vector_iterator<iter>& lhs, const vector_iterator<iter>& rhs) { return (lhs.get_ptr() < rhs.get_ptr()); }
	template<typename iter>
	bool	operator >	(const vector_iterator<iter>& lhs, const vector_iterator<iter>& rhs) { return (rhs < lhs); }
	template<typename iter>
	bool	operator <=	(const vector_iterator<iter>& lhs, const vector_iterator<iter>& rhs) { return !(rhs < lhs); }
	template<typename iter>
	bool	operator >=	(const vector_iterator<iter>& lhs, const vector_iterator<iter>& rhs) { return !(lhs < rhs); }

	template<typename iter1, typename iter2>
	bool	operator ==	(const vector_iterator<iter1>& lhs, const vector_iterator<iter2>& rhs) { return (lhs.get_ptr() == rhs.get_ptr()); }
	template<typename iter1, typename iter2>
	bool	operator !=	(const vector_iterator<iter1>& lhs, const vector_iterator<iter2>& rhs) { return (lhs.get_ptr() != rhs.get_ptr()); }
	template<typename iter1, typename iter2>
	bool	operator <	(const vector_iterator<iter1>& lhs, const vector_iterator<iter2>& rhs) { return (lhs.get_ptr() < rhs.get_ptr()); }
	template<typename iter1, typename iter2>
	bool	operator >	(const vector_iterator<iter1>& lhs, const vector_iterator<iter2>& rhs) { return (rhs < lhs); }
	template<typename iter1, typename iter2>
	bool	operator <=	(const vector_iterator<iter1>& lhs, const vector_iterator<iter2>& rhs) { return !(rhs < lhs); }
	template<typename iter1, typename iter2>
	bool	operator >=	(const vector_iterator<iter1>& lhs, const vector_iterator<iter2>& rhs) { return !(lhs < rhs); }


	template<typename iter>
	bool	operator ==	(const const_vector_iterator<iter>& lhs, const const_vector_iterator<iter>& rhs) { return (lhs.get_ptr() == rhs.get_ptr()); }
	template<typename iter>
	bool	operator !=	(const const_vector_iterator<iter>& lhs, const const_vector_iterator<iter>& rhs) { return (lhs.get_ptr() != rhs.get_ptr()); }
	template<typename iter>
	bool	operator <	(const const_vector_iterator<iter>& lhs, const const_vector_iterator<iter>& rhs) { return (lhs.get_ptr() < rhs.get_ptr()); }
	template<typename iter>
	bool	operator >	(const const_vector_iterator<iter>& lhs, const const_vector_iterator<iter>& rhs) { return (rhs < lhs); }
	template<typename iter>
	bool	operator <=	(const const_vector_iterator<iter>& lhs, const const_vector_iterator<iter>& rhs) { return !(rhs < lhs); }
	template<typename iter>
	bool	operator >=	(const const_vector_iterator<iter>& lhs, const const_vector_iterator<iter>& rhs) { return !(lhs < rhs); }

	template<typename iter1, typename iter2>
	bool	operator ==	(const const_vector_iterator<iter1>& lhs, const const_vector_iterator<iter2>& rhs) { return (lhs.get_ptr() == rhs.get_ptr()); }
	template<typename iter1, typename iter2>
	bool	operator !=	(const const_vector_iterator<iter1>& lhs, const const_vector_iterator<iter2>& rhs) { return (lhs.get_ptr() != rhs.get_ptr()); }
	template<typename iter1, typename iter2>
	bool	operator <	(const const_vector_iterator<iter1>& lhs, const const_vector_iterator<iter2>& rhs) { return (lhs.get_ptr() < rhs.get_ptr()); }
	template<typename iter1, typename iter2>
	bool	operator >	(const const_vector_iterator<iter1>& lhs, const const_vector_iterator<iter2>& rhs) { return (rhs < lhs); }
	template<typename iter1, typename iter2>
	bool	operator <=	(const const_vector_iterator<iter1>& lhs, const const_vector_iterator<iter2>& rhs) { return !(rhs < lhs); }
	template<typename iter1, typename iter2>
	bool	operator >=	(const const_vector_iterator<iter1>& lhs, const const_vector_iterator<iter2>& rhs) { return !(lhs < rhs); }
